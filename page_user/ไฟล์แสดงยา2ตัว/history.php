...
try {
    // Fetch OCR history including the second set of data
    $stmt = $db->prepare("
        SELECT id, ocr_scans_text, ocr_image_data, login_time, medicine_alert_time, 
               ocr_scans_text2, ocr_image_data2, medicine_alert_time2, access_token2, image2
        FROM users
...
<body>
    ...
    <div class="container">
        ...
        <h4 class="mt-2 mb-2 text-white">ข้อมูลยาตัวที่ 1</h4>
        ...
        <!-- Card 2 for 'ocr_scans_text2' and 'medicine_alert_time2' -->
        <h4 class="mt-4 mb-2 text-white">ข้อมูลยาตัวที่ 2</h4>
        <?php foreach ($ocrHistory as $entry) : ?>
            <div class="card">
                <div class="card-body">
                    ...
                    <?php endif; ?>
                    <div>
                        <p class="card-text" id="ocrText2<?= $entry['id'] ?>"><?= isset($entry['ocr_scans_text2']) ? htmlspecialchars($entry['ocr_scans_text2']) : 'No data available' ?></p>
                        <p class="text-muted"><?= isset($entry['login_time']) ? date('F j, Y, g:i a', strtotime($entry['login_time'])) : 'No date available' ?></p>
                        <p class="text-muted">เวลาที่แจ้งเตือน: <?= isset($entry['medicine_alert_time2']) ? htmlspecialchars($entry['medicine_alert_time2']) : 'No time available' ?></p>
                        <?php if (!empty($entry['ocr_scans_text2'])) : ?>
                            <button type="button" class="btn btn-primary btn-sm" onclick="editOCR2(<?= $entry['id'] ?>)">แก้ไข</button>
                            <button type="button" class="btn btn-danger btn-sm" onclick="deleteOCR2(<?= $entry['id'] ?>)">ลบ</button>
                            <button type="button" class="btn btn-info btn-sm" onclick="showUploadForm2(<?= $entry['id'] ?>)">แจ้งเตือนผ่าน LINE</button>
                            <!-- tokenทดสอบ t1VVF2xuiQUoBKTrOkcFOtvzj9Yjptiq6ixUNIIdvgv -->
                        <?php endif; ?>
                    </div>
                </div>
    ...
    function showUploadForm2(ocrId) {
        const ocrEntry = <?= json_encode($ocrHistory) ?>.find(entry => entry.id == ocrId);
        const message = `รายการยา: ${ocrEntry.ocr_scans_text2 || 'No data available'}`;
        $('#ocrId').val(ocrId);
        $('#messagePreview').val(message);
        $('#uploadModal').modal('show');
    }
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: 'update_ocr.php',
                    method: 'POST',
                    data: { id: id, ocr_text: result.value, slot: 'slot1' },
    ...
    function editOCR2(id) {
        const ocrText = document.getElementById(`ocrText2${id}`).innerText;
        Swal.fire({
            title: 'แก้ไขข้อความตัวยา',
            input: 'textarea',
            inputLabel: 'ข้อความตัวยา',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: 'update_ocr.php',
                    method: 'POST',
                    data: { id: id, ocr_text: result.value, slot: 'slot2' },
                    success: function(response) {
                        Swal.fire({
                            icon: 'success',
                            title: 'บันทึกข้อมูลเรียบร้อยแล้ว',
